...
def front_x(words):
    ...
    return list(i for i in words if i[0] == 'x')+list(i for i in words if i[0] != 'x')#x_words+not_x_words
...
def sort_last(lists):
    lists.sort(key=lambda val: val[-1])
    return lists
...
def remove_adjacent(nums):
    return [nums[i] for i in range(len(nums)) if nums[i] != (nums[(i-1)] if i != 0 else '')]
