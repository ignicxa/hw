...
def donuts(count):
    return 'Количество пончиков: много' if (count >= 10) else 'Количество пончиков: ' + str(count)
...
# Этот ужин хорош!
def not_bad(s):
    #no_pos, bad_pos = s.find('не'), s.find('плох')
    #if no_pos < bad_pos:
    #    return s.replace(s[no_pos:bad_pos+4], 'хорош')
    #else:
    #    return s
    return s.replace(s[s.find('не'):s.find('плох')+4], 'хорош') if s.find('не') < s.find('плох') else s
...
def front_back(a, b):
    middle_a, middle_b = round(((len(a)+0.1)/2)), round(((len(b)+0.1)/2))
